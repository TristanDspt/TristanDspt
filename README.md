# Bonjour, je suis Tristan Despont 👋

🎯 **Data Analyst en reconversion** — 15 ans d'expérience en management opérationnel et optimisation logistique, aujourd'hui passionné par la data et le Machine Learning.

---

## 🧰 Stack technique

**Langages & données**
`Python` `SQL` `DBT`

**ML & Data Science**
`Scikit-learn` `Pandas` `Numpy` `EDA` `Classification` `Clustering` `Régression`

**Visualisation & BI**
`Streamlit` `Plotly` `Seaborn` `Power BI`

**Environnement**
`Git` `GitHub` `VS Code` `DBeaver` `PostgreSQL`

---

## 📂 Projets phares

### 🔍 [Détection de faux billets — ONCFM](https://github.com/TristanDspt/Portefolio-OpenClassrooms/tree/master/P12%20-%20Detection%20de%20faux%20billets)
Comparaison de 4 algorithmes de classification (Régression Logistique, KNN, Random Forest, KMeans) pour identifier automatiquement les faux billets à partir de 6 dimensions géométriques.
- Analyse exploratoire complète (EDA)
- Évaluation via matrices de confusion, précision, recall
- Modèle final exporté et prêt à la production (`.pkl`)
- `Scikit-learn` `Pandas` `Python`

---

### 🍷 [Segmentation client — BottleNeck](https://github.com/TristanDspt/Portefolio-OpenClassrooms/tree/master/P13%20-%20Piloter%20un%20projet%20data%20augmenté%20avec%20l'IA)
Segmentation non supervisée d'un portefeuille de vins (marchand en ligne) sur 4 axes : prix, ventes, ancienneté, rotation des stocks.
- Comparaison KMeans vs Clustering Agglomératif
- Identification d'un segment "Stock dormant" invisible en analyse bivariée
- Pipeline reproductible avec Parquet
- `Scikit-learn` `Pandas` `Python`

---

### 📈 [Pilotage de Portefeuille Financier](https://github.com/TristanDspt/Personnal-Finance-Tracker)
Application de suivi de portefeuille boursier en temps réel, de bout en bout.
- Backend : Architecture PostgreSQL & pipeline ETL automatisé via API (yFinance)
- Frontend : Web-App interactive sous Streamlit
- Dataviz : Dashboard de performance en temps réel
- `Python` `PostgreSQL` `Streamlit` `Plotly`

---

## 🎓 Certifications

| Certification | Organisme | Statut |
|---|---|---|
| RNCP 37837 — Data Analyst (Option Statistiques) | OpenClassrooms | ✅ Obtenu — Juillet 2026 |

---

## 📬 Me contacter

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Tristan_Despont-blue?logo=linkedin)](https://linkedin.com/in/tristan-despont)
[![Email](https://img.shields.io/badge/Email-tristan.despont@hotmail.fr-red?logo=gmail)](mailto:tristan.despont@hotmail.fr)

> 📍 Tours et ses environs — Ouvert au remote
